...
nome = 'luis_henrique_lima_santos' # # coloque aqui o nome completo sem espaços (colocar '_' entre as palavras e.g. gustavo_voltani_von_atzingen) no formato de uma string
CPF = '70187298408' # Coloque seu RG no formato int (sem pontos, traços ou espaços) ex: Se '123.456.789.7':  CPF = 1234567897
data_nascimento = '06/03/2002' # Coloque sua data de nascimento no formato 'dd/mm/aaaa' (string)
...
def primo(n):
    ...
    for primo in range(2, int(sqrt(n)) + 1):
        if n % primo == 0:
            return resultado
    resultado = True
    ### Seu código termina aqui ###
...
def fit_linear(x, y):
    ...
    ### Seu código inicia aqui ###	
    n = len(x)
    B = (n * sum(list(map(lambda i, j: i * j, x, y))) - sum(x) * sum(y)) / (n * sum(list(map(lambda k: k ** 2, x))) - sum(x) ** 2)
    A = (sum(y) - B * sum(x)) / n
    ### Seu código termina aqui ###
    return A, B
